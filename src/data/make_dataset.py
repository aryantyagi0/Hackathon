...
def main():
    ...
    booking_df = store.get_raw("booking_log.csv")
    print("Loaded booking rows:", len(booking_df))
    print("Booking DF columns:", booking_df.columns)

    booking_df = clean_booking_df(booking_df)

    participant_df = store.get_raw("participant_log.csv")
    print("Loaded participant rows:", len(participant_df))
    print("Participant DF columns:", participant_df.columns)
